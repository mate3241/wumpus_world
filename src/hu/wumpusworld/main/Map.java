...
public class Map implements java.io.Serializable {
  ...
  public void printMap() {
    for (int i = 0; i < squares.length; i++) {
      ...
      if (i == 0) {
        System.out.print("   Arrow(s): " + player.getArrow());
      }
      if (i == 1) {
        System.out.print("   Meat: " + player.getMeat());
      }
      System.out.println();
    }
  }
  ...
  public String getWarningsFromNeighbors(Square square) {
    ...
    for (int i = -1; i <= 1; i++) {
      for (int j = -1; j <= 1; j++) {
        ...
        if (isValidNeighbor(square, i, j) && difficulty == Difficulty.HARD &&
                squares[square.getX() + i][square.getY() + j].getMeat().isPresent()) {
          System.out.println(squares[square.getX() + i][square.getY() + j].getMeat().get().getWarning());
          sb.append(squares[square.getX() + i][square.getY() + j].getMeat().get().getWarning());
          sb.append("\n");
        }
      }
    }
